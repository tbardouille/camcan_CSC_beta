...
def MEG_preproc(subjectID):
    ...
    # Now drop button presses that are within 3 seconds of the previous button
    # press
    goodButtonInterval = np.diff(goodButtonEvents[:, 0])/raw.info['sfreq']
    longDelay = np.where(goodButtonInterval > 3)[0]

    # Write out event files for all stimuli/responses and "good" button presses
    # only
    ...
    epochs = mne.Epochs(raw, np.array(goodButtonEvents), None, prestim,
                        poststim, baseline=(baseStart, baseEnd), verbose=False,
                        preload=True)
