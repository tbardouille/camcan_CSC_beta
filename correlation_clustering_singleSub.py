...
for subID in subjects:
    ...
    if os.path.exists(fileName):
        ...
        atomGroups = pd.DataFrame(
            columns=['Subject ID', 'Atom number', 'Group number'])
        ...
        groupDict = {'Subject ID': subID,
                     'Atom number': atomNum, 'Group number': groupNum}
        ...
        for atom in range(1, cdl_model.u_hat_.shape[0]):
            ...
            if (unique == False):
                groupDict = {'Subject ID': subID,
                             'Atom number': atomNum, 'Group number': max_group}
        ...
        groupSummary = pd.DataFrame(
            columns=['Group Number', 'Number of Atoms'])
