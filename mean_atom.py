...
# For a given class, plot all topomaps accross the subjects
method = 'cah'
class_label = 5
sub_df = df_topomaps[df_topomaps['labels_' + method] == class_label]
for subject_id in list(set(sub_df['subject_id'].values)):
    print('subject id %s' % subject_id)
    atom_idx = sub_df[sub_df['subject_id'] ==
                      subject_id]['atom_id'].values.astype(int)
    plot_topomaps(subject_id, atom_idx)

# %%
compute_mean_atom(df_topomaps, use_batch_cdl=False)
