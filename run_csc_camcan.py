...
from config import BIDS_ROOT, SSS_CAL_FILE, CT_SPARSE_FILE, PARTICIPANTS_FILE, HOME_DIR
...
for subject_id in subjects:
    ...
    currPath = '/media/NAS/lpower/CSC/results/' + \
        subject_id + '/CSCraw_0.5s_20atoms.pkl'
    ...
    if test_id in participants['participant_id'].tolist():
        ...
        if go:  # os.path.exists(testPath):
            if go:  # not os.path.exists(currPath):
                ...
                print(
                    f'Running CSC pipeline on: {subject_id}, {str(age)} year old {sex}')
  ...
  #  fig = evokeds.plot_joint(picks="grad")
   # fig_name = "evokeds.pdf"
    #fig.savefig(subject_output_dir / fig_name, dpi=300)
                ...
                pkl_name = 'CSCraw_' + \
                    str(atom_dur) + 's_' + str(n_atoms) + 'atoms.pkl'
                outputFile = subject_output_dir / pkl_name

                info = raw.info
                pickle.dump([cdl_model, info, allZ, z_hat_],
                            open(outputFile, "wb"))
...
# %%
                ...
                bemFif = subjectsDir + '/sub-' + subject_id + \
                ...
                for atom in range(0, n_atoms):
                    ...
                    pre = allZ[:, atom, 68:218]
                    # -0.25 to 0.25 sec (75 samples)
                    move = allZ[:, atom, 218:293]
                    # 0.25 to 1.25 sec (150 samples)
                    post = allZ[:, atom, 293:443]
                    ...
                    for atom2 in range(0, cdl_model.u_hat_.shape[0]):
                        ...
                        corrcoef = np.corrcoef(
                            cdl_model.u_hat_[atom], cdl_model.u_hat_[atom2])[0, 1]
                        u_coefs.append(abs(corrcoef))

                        # calculate and save psd coefficients to list
                        psd1 = np.abs(np.fft.rfft(
                            cdl_model.v_hat_[atom], n=256)) ** 2
                        psd2 = np.abs(np.fft.rfft(
                            cdl_model.v_hat_[atom2], n=256)) ** 2
