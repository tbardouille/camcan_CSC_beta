...
# Path to where save results (current working directory or home)
HOME_DIR = Path.cwd()
# HOME_DIR = Path.home()
...
def meg_preproc(subject_id, apply_maxwell_filter=True,
    ...
    raw = read_raw_bids(bids_path=bp, extra_params={'preload': True})
...
@memory.cache()
def run_csc(subject_id, apply_maxwell_filter=True, apply_ica_cleaning=False,
            subtract_first_samp=True,  cdl_on_epoch=False,
            n_atoms=25, atom_duration=0.7,
    ...
    cdl_on_epoch : bool
        if True, run CDL on epoched data,
        if False, run CDL on ful length data
        default is False
    ...
    res = [cdl_meg, info, z_hat_]
        ...
        cdl_meg : instance of BatchCDL/GreedyCDL
    ...
    epochs, evoked, raw = meg_preproc(
        subject_id=subject_id, apply_maxwell_filter=apply_maxwell_filter,
        apply_ica_cleaning=apply_ica_cleaning)
    ...
    if cdl_on_epoch:
        ...
        _, good_buttons_events = get_good_buttons_events(
    ...
    if cdl_on_epoch:
        ...
        res = [cdl_meg, info, z_hat_, good_buttons_events]
...
def get_exp_name(subject_id, use_batch_cdl=True, use_greedy_cdl=False,
                 cdl_on_epoch=False, atom_duration=0.7, sensorType='grad',
                 n_atoms=25, reg=.2, eps=1e-4, tol_z=1e-2, **kwargs):
    """
    XXX
    """
    assert (use_batch_cdl + use_greedy_cdl) == 1

    exp_name = subject_id + '_'
    ...
    exp_name += str(int(atom_duration * 1000)) + \
...
def plot_csc(exp_params, activeStartTime=1.7, shift_acti=False):
    """

    Parameters
    ----------
    exp_params : dict
        the dictionnary with all the pre-processing and CSC parameters
        (e.g., subject_id, sfreq, n_atoms, etc.)
    ...
    sfreq = exp_params.get('sfreq', 150.)
    preStimStartTime = 0
    ...
    figNameSuffix = get_exp_name(**exp_params) \
        + '_age' + str(subject_age) + '.pdf'
    ...
    if exp_params.get('cdl_on_epoch', False):
        cdl, info, z_hat_ = run_csc(**exp_params)
        allZ = z_hat_
    else:
        cdl, info, z_hat_, good_button_events = run_csc(**exp_params)
        # transform z_hat_ into epoched, with shape (n_events, n_atoms, n_tt)
        # file with good button events
        # eveFif_button = subject_output_dir / \
        #     (dsPrefix + '_Under2SecResponseOnly-eve.fif')
        # goodButtonEvents = mne.read_events(eveFif_button)
        print("Number of events: %i" % len(good_button_events))
        info['events'] = np.array(good_button_events)
        info['event_id'] = None
    ...
    plt.xlabel('Atom #')
    plt.savefig(subject_output_dir / ('change_in_activation' + figNameSuffix),
                dpi=300)
    plt.close()
    ...
    plt.ylabel('t-stat')
    plt.xlabel('Atom #')
    plt.savefig(subject_output_dir / ('t_test' + figNameSuffix), dpi=300)
    plt.close()
    ...
    plt.savefig(subject_output_dir / fig_name, dpi=300)
    plt.savefig(subject_output_dir /
                (fig_name.replace('.pdf', '.png')), dpi=300)
    # bbox_inches='tight')
    plt.close()
